...
def back_translate(sentences: list):
    ...
    for t in sentences:
        # here we protect ourselves from the API failing
        # if we cannot translate, we default to the original sentence and alert the user
        try:
            result = trans.translate(t, src='en', tmp = 'zh-cn').result_text
        except:
            print("===> rror translating sentence: {}, we default to no translation!".format(t))
            result = t 
        finally:
